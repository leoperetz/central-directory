...
Test('Auth Module', moduleTest => {
  moduleTest.test('register should', registerTest => {
    registerTest.test('register dfsp auth strategy', async function (test) {
    ...
    registerTest.test('register token auth strategy', async function (test) {
      let strategySpy = Sinon.spy()
      let server = {
        auth: {
          scheme: Sinon.spy(),
  ...
  moduleTest.test('should be named "auth-strategies"', async function (test) {
